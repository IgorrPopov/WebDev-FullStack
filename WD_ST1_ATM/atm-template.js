...
const INFO = {
    ...
    logged_in: ['You logged in', 'successful login'],
    checked_debet: ['Current debet: ', 'current debet '],
    get_your_money: ['Get your money! Balance: ', 'cash withdrawal, balance: '],
    replenished_balance: ['Account replenished, balance: ', 'account replenished, balance: '],
    replenished_atm_balance: ['AMT cash balance increased to: ', 'AMT cash balance increased to: '],
    get_report: ['___________________________________', 'got report'],
    logged_out: ['Logged out', 'logged out']
};
/* colors of messages */
...
const ATM = {
    ...
    auth: function(number, pin) {
        this.addReport(checks(['isLogged', 'isUserInputString',
            ...
            log('auth', 'logged_in') : log('auth', current_error));
    },
    // check current debet
    check: function() {
        this.addReport(checks(['isAuth'], [this]) ?
            log('check', 'checked_debet', this.current_user.debet) :
    ...
    getCash: function(amount) {
        const gettingCash = (amount) => {
            ...
            return log('getCash', 'get_your_money', this.current_user.debet);
        };
        this.addReport(checks(['isAuth', 'isNotAdmin', 'isValidAmount',
    ...
    loadCash: function(amount){
        const loadAccount = (amount) => {
            this.cash += Number(amount);
            this.current_user.debet += Number(amount);
            return log('loadCash', 'replenished_balance', this.current_user.debet);
        };
        this.addReport(checks(['isAuth', 'isNotAdmin', 'isValidAmount'],
    ...
    load_cash: function(addition) {
        const loadToATM = (addition) => {
            ...
            return log('load_cash', 'replenished_atm_balance', this.cash);
        };
        this.addReport(checks(['isAuth', 'isNotUser', 'isValidAmount'],
            [this, addition]) ? loadToATM(addition) : log('load_cash', current_error));
    },
    // get_report about cash actions - available for admin only - EXTENDED
    getReport: function() {
        const reports = () => this.reports.forEach(
            (element, i) => log('', `${++i}) ${element.getReport()}\n`, REPORT_COLOR));
        this.addReport(checks(['isAuth', 'isNotUser'], [this]) ? (reports(),
            log('getReport', 'get_report')) :
            log('getReport', current_error));
    },
    // log out
    logout: function() {
        if(checks(['isAuth'], [this])) {
            this.addReport(log('logout', 'logged_out'));
...
function log(action, massage, debet = -1) {
    ...
    if(action === ''){ // reports
        ...
        console.log(`%c${INFO[massage][0]}`, `color: ${color}`);
        return ACTIONS[action] + ': ' + INFO[massage][1];
    } else { // with numbers
        console.log(`%c${INFO[massage][0]}${debet}`, `color: ${color}`);
        return ACTIONS[action] + ': ' + INFO[massage][1] + debet;
    }
}
